...
with t5:
    try:
        ...
        # 1. BAGIAN ATAP KALEM (Muted Roof Matrix Row)
        html_code += '<tr>'
        html_code += '<td style="border: none; background: transparent;"></td>' 
        html_code += '<td style="border: none; background: transparent; font-weight: 600; color: #64748b; text-align: right; vertical-align: middle; font-size: 13px;">🛖 Korelasi Atap →</td>'
        for col_target in hows_list:
            roof_correlations = []
            for col_source in hows_list:
                if col_target != col_source:
                    val = st.session_state.roof_matrix.at[col_source, col_target]
                    if "Strong Positive" in val: roof_correlations.append("++")
                    elif "Positive" in val: roof_correlations.append("+")
                    elif "Strong Negative" in val: roof_correlations.append("--")
                    elif "Negative" in val: roof_correlations.append("-")
            
            roof_display = ", ".join(roof_correlations) if roof_correlations else "0"
            # Warna atap diganti ke abu-abu lembut / soft slate
            html_code += f'<td style="background-color: #f1f5f9; color: #475569; font-weight: 600; font-size: 13px;">{roof_display}</td>'
        html_code += '</tr>'
