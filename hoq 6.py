...
# CSS Custom untuk menggambar Rumah HoQ di Tab 5
st.markdown("""
    <style>
...
# --- TAB 5: THE FINAL HOUSE (BENTO COMPACT HOUSE) ---
with t5:
    try:
        ...
        # TAMPILAN 1: BANGUNAN UTAMA RUMAH HOQ (HTML Rendering)
        st.write("### 📊 Kompleks Kisi-Kisi Matriks HoQ Terintegrasi")
        
        # Mulai membangun string HTML tabel
        html_code = '<table class="hoq-table">'
        
        # Header Baris 1: Kolom HOWs (Technical Requirements)
        html_code += '<tr>'
        ...
        # Isian Badan Rumah: Baris WHATs vs Kolom HOWs
        for idx, row_name in enumerate(whats_list):
            ...
            for col_name in hows_list:
                ...
                # Modifikasi visual warna sel berdasarkan kekuatan hubungan (0, 1, 3, 9)
                bg_cell = ""
                if score_val == 9: bg_cell = 'style="background-color: #fecdd3; font-weight: bold;"' # Merah Muda (Kuat)
                elif score_val == 3: bg_cell = 'style="background-color: #fed7aa;"' # Oranye (Sedang)
                elif score_val == 1: bg_cell = 'style="background-color: #fef08a;"' # Kuning (Lemah)
                
                html_code += f'<td {bg_cell}>{int(score_val)}</td>'
            html_code += '</tr>'
            
        # Fondasi Rumah Baris 1: Nilai Absolut (Absolute Importance)
        ...
        # Fondasi Rumah Baris 2: Nilai Relatif (Relative Weight %)
