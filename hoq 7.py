...
with t5:
    try:
        ...
        # -------------------------------------------------------------
        # 1. VISUALISASI MATRIKS ATAP UTAH (HOWs vs HOWs) - KALEM
        # -------------------------------------------------------------
        ...
        for row_name in hows_list:
            html_roof += '<tr>'
            html_roof += f'<td class="hoq-td-whats" style="background-color: #f1f5f9; color: #1e3a8a;">{row_name}</td>'
            for col_name in hows_list:
                val = st.session_state.roof_matrix.at[row_name, col_name]
                
                # Standarisasi display simbol agar ringkas di tabel
                simbol = "0"
                bg_cell = ""
                if "Strong Positive" in val: 
                    simbol = "++"
                    bg_cell = 'style="background-color: #e0f2fe; color: #0369a1; font-weight: 600;"' # Soft Blue
                elif "Positive" in val: 
                    simbol = "+"
                    bg_cell = 'style="background-color: #f0fdf4; color: #15803d; font-weight: 600;"' # Soft Green
                elif "Strong Negative" in val: 
                    simbol = "--"
                    bg_cell = 'style="background-color: #fef2f2; color: #b91c1c; font-weight: 600;"' # Soft Red
                elif "Negative" in val: 
                    simbol = "-"
                    bg_cell = 'style="background-color: #fff7ed; color: #c2410c; font-weight: 600;"' # Soft Orange
        ...
        # -------------------------------------------------------------
        # 2. VISUALISASI BADAN & FONDASI RUMAH HOQ - KALEM
        # -------------------------------------------------------------
        ...
        for idx, row_name in enumerate(whats_list):
            ...
            for col_name in hows_list:
                ...
                bg_cell = ""
                # Warna pastel sangat soft agar tidak terlalu kontras
        ...
        html_body += '<td style="text-align: right;">Absolute Importance (Score)</td>'
        html_body += '<td>-</td>'
        ...
        html_body += '<td style="text-align: right;">Relative Weight (%)</td>'
        html_body += '<td>-</td>'
