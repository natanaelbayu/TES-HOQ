...
with t1:
    ...
    st.session_state.whats = st.data_editor(
        st.session_state.whats, 
        num_rows="dynamic", 
...
with t2:
    ...
    st.session_state.hows = st.data_editor(
        st.session_state.hows, 
        num_rows="dynamic", 
        use_container_width=True, 
        key="ed_hows"
    )

# Mengambil list terbaru setelah user mengubah data di dashboard (Tab 1 & Tab 2)
# Ditambahkan filter agar baris kosong yang baru dibuat tidak bikin error
whats_list = [x for x in st.session_state.whats["Customer Requirement (WHATs)"].tolist() if pd.notna(x) and x != ""]
hows_list = [x for x in st.session_state.hows["Technical Requirement (HOWs)"].tolist() if pd.notna(x) and x != ""]


# TAB 3 (Correlation - The Roof)
with t3:
    st.subheader("Matriks Korelasi Atap (HOWs vs HOWs)")
    # Cek atau buat ulang matriks atap jika ada perubahan jumlah data HOWs di dashboard
...
with t4:
    ...
    st.caption("Isi dengan nilai: 9 (Kuat), 3 (Sedang), 1 (Lemah), atau 0 jika tidak berhubungan.")
    
    # Cek atau buat ulang matriks jika ada perubahan data WHATs atau HOWs di dashboard
    if 'rel_matrix' not in st.session_state or list(st.session_state.rel_matrix.columns) != hows_list or list(st.session_state.rel_matrix.index) != whats_list:
        st.session_state.rel_matrix = pd.DataFrame(0.0, index=whats_list, columns=hows_list)
...
with t5:
    try:
        # LOGIKA HITUNG (Mengambil data dinamis yang sudah diinput/edit di dashboard)
