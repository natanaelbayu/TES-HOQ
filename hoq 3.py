...
with t2:
    ...
    st.caption("Isi dengan arah spesifikasi teknis: Max (maksimum), Min (minimum), atau Target (target).")
...
with t3:
    ...
    roof_df = pd.DataFrame("", index=hows_list, columns=hows_list)
    
    # Mengedit korelasi atap
...
with t4:
    ...
    # Buat dataframe kosongan baru sesuai WHATs dan HOWs yang aktif (jika diapus otomatis mengecil)
    rel_df = pd.DataFrame(0.0, index=whats_list, columns=hows_list)
...
with t5:
    try:
        # Mengambil nilai bobot pentingnya (Importance) dari data terbaru di Tab 1
        weights = whats_data["Importance (1-5)"].values
        
        # Mengambil matriks nilai terbaru dari Tab 4
        ...
        with col_voc:
            ...
            st.dataframe(whats_data, hide_index=True, use_container_width=True)
        with col_mat:
