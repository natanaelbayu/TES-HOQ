...
# 1. SETTING HALAMAN & STYLE HOQ RUMAH (Tetap seperti kode Anda)
...
# =====================================================================
# 2. INISIALISASI DATA AWAL (Hanya berjalan SEKALI saat web pertama dibuka)
# =====================================================================
if "ed_whats" not in st.session_state:
    st.session_state["ed_whats"] = pd.DataFrame({
...
if "ed_hows" not in st.session_state:
    st.session_state["ed_hows"] = pd.DataFrame({
        "Technical Requirement (HOWs)": ["Tekanan Mesin Pres", "Kualitas Kedelai", "Lama Perebusan", "Takaran Kunyit"],
        "Direction": ["Max", "Max", "Target", "Max"]
    })

# Mengambil data terbaru secara aman dari state internal editor
df_whats_current = st.session_state["ed_whats"]
df_hows_current = st.session_state["ed_hows"]

# Filter list dinamis agar sel kosong tidak ikut terbaca
whats_list = [x for x in df_whats_current["Customer Requirement (WHATs)"].tolist() if pd.notna(x) and x != ""]
hows_list = [x for x in df_hows_current["Technical Requirement (HOWs)"].tolist() if pd.notna(x) and x != ""]


# --- INISIALISASI MATRIKS LANJUTAN YANG AMAN ---
if "ed_roof" not in st.session_state:
    st.session_state["ed_roof"] = pd.DataFrame("No Correlation (0)", index=hows_list, columns=hows_list)
else:
    # Sinkronisasi dimensi jika user menambah/menghapus item di Tab 2 tanpa merusak data lama
    old_roof = st.session_state["ed_roof"]
    if list(old_roof.columns) != hows_list or list(old_roof.index) != hows_list:
        ...
        st.session_state["ed_roof"] = new_roof

if "ed_rel" not in st.session_state:
    st.session_state["ed_rel"] = pd.DataFrame(0, index=whats_list, columns=hows_list)
else:
    # Sinkronisasi dimensi jika user mengubah data di Tab 1 atau Tab 2 tanpa merusak data lama
    old_rel = st.session_state["ed_rel"]
    if list(old_rel.columns) != hows_list or list(old_rel.index) != whats_list:
        ...
        st.session_state["ed_rel"] = new_rel
...
with t1:
    ...
    st.caption("Isi dengan kebutuhan pelanggan dan bobot pentingnya dari 1 hingga 5. Penambahan baris baru otomatis memperluas matriks.")
    st.data_editor(
        st.session_state["ed_whats"], 
        num_rows="dynamic", 
        use_container_width=True, 
        column_config={
            "Importance (1-5)": st.column_config.NumberColumn("Importance (1-5)", min_value=1, max_value=5, step=1)
        },
        key="ed_whats" # Kunci utama penanganan state otomatis oleh Streamlit
    )
...
with t3:
    ...
    st.data_editor(st.session_state["ed_roof"], use_container_width=True, column_config=roof_column_config, key="ed_roof")
...
with t4:
    ...
    st.info("""
    💡 **Panduan Skor :**
    **9 (Kuat)** | **3 (Sedang)** | **1 (Lemah)** | **0 (Tidak Ada Hubungan)**
    """)
    rel_column_config = {
        col: st.column_config.SelectboxColumn(col, options=[0, 1, 3, 9], required=True)
        for col in hows_list
    }
    st.data_editor(st.session_state["ed_rel"], use_container_width=True, column_config=rel_column_config, key="ed_rel")


# TAB 5: THE FINAL HOUSE & ACTION PLAN
with t5:
    try:
        # Mengambil data fix dari state editor saat ini untuk dikalkulasi
        df_whats_final = st.session_state["ed_whats"]
        valid_whats = df_whats_final[df_whats_final["Customer Requirement (WHATs)"].isin(whats_list)]
        weights = valid_whats["Importance (1-5)"].values.astype(float)
        
        matrix_values = st.session_state["ed_rel"].loc[whats_list, hows_list].values.astype(float)
        ...
        for row_name in hows_list:
            ...
            for col_name in hows_list:
                val = st.session_state["ed_roof"].at[row_name, col_name]
        ...
        for idx, row_name in enumerate(whats_list):
            ...
            for col_name in hows_list:
                score_val = st.session_state["ed_rel"].at[row_name, col_name]
        ...
        for i in range(n_hows - 1):
            ...
            for j in range(n_hows):
                if j < (n_hows - 1 - i): html_hoq += '<td class="roof-blank"></td>'
                    ...
                    val = st.session_state["ed_roof"].at[row_target, col_target]
